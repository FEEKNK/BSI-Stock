...
export function BarcodePage() {
  ...
  const handleSizeQtyChange = (size, delta) => {
    const current = Number(editingSizes[size]) || 0;
    const updated = Math.max(0, current + delta);
    setEditingSizes(prev => ({ ...prev, [size]: updated }));
  };

  const handleSaveStock = async () => {
    if (!scanResult || !scanResult.product) return;
    const totalStock = Object.values(editingSizes).reduce((sum, qty) => sum + (Number(qty) || 0), 0);
                ...
                <div>
                  ...
                  {Object.keys(editingSizes).length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      {Object.entries(editingSizes).map(([size, qty]) => (
                        <div key={size} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 14px', backgroundColor: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
                          <span style={{ fontWeight: 600, color: 'var(--text-primary)' }}>{size}</span>
